...
mod tests {
    ...
    fn parses_race_xml() {
        let path = std::path::Path::new(
            r"C:\Users\michal\dev\racemate\lmu-telemetry\results\2026_03_01_13_56_27-87R1.xml",
        );
    ...
    fn parses_practice_xml() {
        let path = std::path::Path::new(
            r"C:\Users\michal\dev\racemate\lmu-telemetry\results\2026_03_04_05_51_30-39P1.xml",
        );
        let session = parse_xml(path).expect("parse failed");
        assert!(!session.session_blocks.is_empty());
